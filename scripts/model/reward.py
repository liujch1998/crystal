...
from utils.utils import reduce_mean, append_string, append_and_concat
...
class Reward:
    ...
    def __init__(self,
        ...
        self.no_grad_in_qa = no_grad_in_qa
    ...
    def get_reward(self,
        ...
        if knowledges_input_ids is not None and knowledges_attention_mask is not None:
            prompts_input_ids, prompts_attention_mask = append_and_concat(questions_input_ids, questions_attention_mask, ' \\n Knowledge: ', knowledges_input_ids, knowledges_attention_mask, self.tokenizer)
        ...
        qa_loss = output.loss
        logitsss = output.logits # (B, AL, V)
        logitss = logitsss[:, 0, :] # (B, V)
        choice_idss = choicess_input_ids[:, :, 0] # (B, C)
        answer_logitss = logitss.gather(dim=1, index=choice_idss) # (B, C)
        answer_logitss[choice_idss == 1] = -1e9
