...
datapath_by_task_and_split = {
    ...
    'com2sense_paired': {'default': 'com2sense_paired'},
    'sciq': {'default': 'sciq'},
    'quarel': {'default': 'quarel'},
    'quartz': {'default': 'quartz'},
    'cycic_mc': {'default': 'cycic_mc'},
    'comve_a': {'default': 'comve_a'},
    ...
    'hellaswag': {'default': 'hellaswag'},
    'codah': {'default': 'codah'},
    'story_cloze_test': {'default': 'story_cloze_test'},
    'alphanli': {'default': 'alphanli'},
    'riddlesense': {'default': 'riddlesense'},
...
class PPOTrainer:
    ...
    def train(self, step):
        ...
        if not self.args.half_half:
            ...
            if step % (self.args.rl_and_qa_steps) in range(0, self.args.rl_steps):
                try:
        ...
        with torch.no_grad():
            _ = self.policy_model.model(
                input_ids=torch.ones(1, 1).long().to(accelerator.device),
                labels=torch.ones(1, 1).long().to(accelerator.device),
            )
            if not self.args.policy_value_sharing:
                _ = self.value_model.model(
                    input_ids=torch.ones(1, 1).long().to(accelerator.device),
                    labels=torch.ones(1, 1).long().to(accelerator.device),
                )
            if not self.args.policy_reward_sharing:
                _ = self.reward_model.model(
                    input_ids=torch.ones(1, 1).long().to(accelerator.device),
                    labels=torch.ones(1, 1).long().to(accelerator.device),
                )
        ...
        # QKA training
        if step % (self.args.rl_and_qa_steps) in range(self.args.rl_steps, self.args.rl_and_qa_steps):
            # Get reward
            # This could be optimized by setting skip_reward=True, save one reward model pass
        ...
        if step % (self.args.rl_and_qa_steps) in range(0, self.args.rl_steps):
            ...
            for ppo_epoch_idx in range(self.args.noptepochs):
                self.optimizer.zero_grad()
                self.loss(results)
                loss = self.args.rl_coef * results['loss/total']
                accelerator.backward(loss)
    ...
    def valid(self, step):
        ...
        if self.args.eval_interval != 0:
            ...
            if step % self.args.rl_and_qa_steps not in [0, self.args.rl_steps]:
                return
...
def main():
    ...
    if args.mode == 'train':
        ref_policy = Policy(
            ...
            model_ckpt=args.model_ckpt if args.load_from_ckpt is None else None,
            tokenizer=tokenizer,
        ...
        value = Value(
            model_type=args.model_type,
            model_ckpt=args.model_ckpt if (args.load_from_ckpt is None and args.use_model_ckpt_for_value) else None,
        ...
        reward = Reward(
            ...
            no_grad_in_qa=args.no_grad_in_qa,
        )
        ...
        if args.load_from_ckpt is not None:
            ...
            # if not args.policy_value_sharing and args.use_model_ckpt_for_value:
            #     accelerator.unwrap_model(value.model).load_state_dict(checkpoint['model'], strict=False)
            if not args.policy_reward_sharing:
                accelerator.unwrap_model(reward.model).load_state_dict(checkpoint['model'])
            checkpoint.clear()
