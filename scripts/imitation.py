...
from utils.utils import ensure_dir, set_seed, reduce_mean, append_string, concat_strings, append_and_concat
...
class QKADataset(Dataset):
    ...
    def load_datasets(self):
        ...
        for task_ix, task in enumerate(self.tasks):
            path = f'../data/knowledge/knowledge_gkp_gpt3{self.args.engine}.{self.split}.{task}.json'
            with open(path) as f:
                js = json.load(f)
    ...
    def collate_fn(batch):
        ...
        knowledges_tok = tokenizer.batch_encode_plus(
            knowledges,
            return_tensors='pt', padding='max_length', truncation='longest_first', max_length=tokenizer.max_knowledge_len)
        knowledges_input_ids = knowledges_tok.input_ids
        knowledges_attention_mask = knowledges_tok.attention_mask
        knowledges_labels = knowledges_input_ids.clone()
        knowledges_labels[knowledges_attention_mask == 0] = -100
        ...
        return {
            ...
            'knowledges_input_ids': knowledges_input_ids,
            'knowledges_attention_mask': knowledges_attention_mask,
            'knowledges_labels': knowledges_labels,
        }
...
class Trainer:
    ...
    def qk_loss(self, batch):
        input_ids, attention_mask = append_string(batch['questions_input_ids'], batch['questions_attention_mask'], ' \\n Knowledge: ', self.tokenizer)
        # log_info(f'qk_loss, {self.tokenizer.decode(input_ids[0])}')
        loss = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            labels=batch['knowledges_labels'],
        ).loss
        return loss

    def qa_loss(self, batch):
        input_ids, attention_mask = append_string(batch['questions_input_ids'], batch['questions_attention_mask'], ' \\n Answer: ', self.tokenizer)
        # log_info(f'qa_loss, {self.tokenizer.decode(input_ids[0])}')
        loss = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            labels=batch['answers_labels'],
        ).loss
        return loss

    def qka_loss(self, batch):
        input_ids, attention_mask = append_and_concat(batch['questions_input_ids'], batch['questions_attention_mask'], ' \\n Knowledge: ', batch['knowledges_input_ids'], batch['knowledges_attention_mask'], self.tokenizer)
        input_ids, attention_mask = append_string(input_ids, attention_mask, ' \\n Answer: ', self.tokenizer)
        # log_info(f'qka_loss, {self.tokenizer.decode(input_ids[0])}')
        loss = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            labels=batch['answers_labels'],
    ...
    def train(self, step):
        ...
        for _ in range(self.args.accumulate_grad_batches):
            try:
                ...
                self.train_sampler = iter(self.train_dataloader)
                batch = next(self.train_sampler)
            if self.args.qa_loss_multiplier != 0.0 and self.args.qk_loss_multiplier == 0.0 and self.args.qka_loss_multiplier == 0.0:
                qa_loss = self.qa_loss(batch)
                qk_loss = torch.tensor(0.0, device=qa_loss.device, dtype=qa_loss.dtype, requires_grad=False)
                qka_loss = torch.tensor(0.0, device=qa_loss.device, dtype=qa_loss.dtype, requires_grad=False)
            elif self.args.qa_loss_multiplier != 0.0 and self.args.qk_loss_multiplier != 0.0 and self.args.qka_loss_multiplier != 0.0:
                qa_loss = self.qa_loss(batch)
                qk_loss = self.qk_loss(batch)
                qka_loss = self.qka_loss(batch)
            elif step % self.args.qk_and_qka_steps in range(0, self.args.qk_steps):
                qk_loss = self.qk_loss(batch)
                qa_loss = torch.tensor(0.0, device=qk_loss.device, dtype=qk_loss.dtype, requires_grad=False)
                qka_loss = torch.tensor(0.0, device=qk_loss.device, dtype=qk_loss.dtype, requires_grad=False)
            elif step % self.args.qk_and_qka_steps in range(self.args.qk_steps, self.args.qk_and_qka_steps):
                qka_loss = self.qka_loss(batch)
                qk_loss = torch.tensor(0.0, device=qka_loss.device, dtype=qka_loss.dtype, requires_grad=False)
                qa_loss = torch.tensor(0.0, device=qka_loss.device, dtype=qka_loss.dtype, requires_grad=False)
            loss = qk_loss * self.args.qk_loss_multiplier + qa_loss * self.args.qa_loss_multiplier + qka_loss * self.args.qka_loss_multiplier
    ...
    def acc(self, batch, use_knowledge=False):
        if not use_knowledge:
            prompts_input_ids, prompts_attention_mask = append_string(batch['questions_input_ids'], batch['questions_attention_mask'], ' \\n Answer: ', self.tokenizer) # (B, QL)
        else:
            prompts_input_ids, prompts_attention_mask = append_and_concat(batch['questions_input_ids'], batch['questions_attention_mask'], ' \\n Knowledge: ', batch['knowledges_input_ids'], batch['knowledges_attention_mask'], self.tokenizer)
            prompts_input_ids, prompts_attention_mask = append_string(prompts_input_ids, prompts_attention_mask, ' \\n Answer: ', self.tokenizer) # (B, QL + KL)
        choicess_input_ids = batch['choicess_input_ids'] # (B, C, AL)
        choicess_attention_mask = batch['choicess_attention_mask'] # (B, C, AL)
        choicess_labels = batch['choicess_labels'] # (B, C, AL)
        answer_ixs = batch['answer_ixs'] # (B)

        # Compute number of choices for each question, and flatten prompts accordingly
        flattened_prompts_input_ids = torch.repeat_interleave(prompts_input_ids, choicess_input_ids.size(1), dim=0) # (B * C, QL)
        flattened_prompts_attention_mask = torch.repeat_interleave(prompts_attention_mask, choicess_input_ids.size(1), dim=0) # (B * C, QL)
        ...
        for i in range(0, flattened_prompts_input_ids.size(0), self.args.batch_size):
            j = min(i + self.args.batch_size, flattened_prompts_input_ids.size(0))
        ...
        all_losses = all_losses.view(prompts_input_ids.size(0), -1) # (B, C)
    ...
    def valid(self, step):
        ...
        if self.args.eval_interval != 0:
            ...
            if step % self.args.qk_and_qka_steps not in [0]: # only valid after qka steps
                return
        if step in self.eval_losses or step in self.eval_accs:
            return
        ...
        with torch.no_grad():
            losses, qk_losses, qa_losses, qka_losses = [], [], [], []
            corrects, corrects_knowful, task_ixs = [], [], []
            for i, batch in enumerate(tqdm(self.eval_dataloader) if accelerator.is_main_process else self.eval_dataloader):
                if i == self.args.eval_loop_cap:
                    break
                qk_loss = self.qk_loss(batch)
                qa_loss = self.qa_loss(batch) if self.args.qa_loss_multiplier != 0.0 else torch.tensor(0.0, device=qk_loss.device, dtype=qk_loss.dtype, requires_grad=False)
                qka_loss = self.qka_loss(batch)
                loss = qk_loss * self.args.qk_loss_multiplier + qa_loss * self.args.qa_loss_multiplier + qka_loss * self.args.qka_loss_multiplier
                results = self.acc(batch)
                results_knowful = self.acc(batch, use_knowledge=True)
                losses.append(loss.detach().clone())
                qk_losses.append(qk_loss.detach().clone())
                qa_losses.append(qa_loss.detach().clone())
                qka_losses.append(qka_loss.detach().clone())
                corrects.append(results['corrects'])
                corrects_knowful.append(results_knowful['corrects'])
        ...
        corrects_knowful = torch.stack(corrects_knowful, dim=0) # (M, B)
        ...
        corrects_knowful = accelerator.gather(corrects_knowful.unsqueeze(0)) # (num_gpus, M, B)
        ...
        corrects_knowful = corrects_knowful.transpose(0, 1).flatten(0, 2)[:len(self.eval_dataloader.dataset)] # (N)
        ...
        corrects_knowful_by_task = defaultdict(list)
        for task_ix, correct in zip(task_ixs, corrects_knowful):
            task = self.eval_dataloader.dataset.tasks[task_ix]
            corrects_knowful_by_task[task].append(correct)
        corrects_knowful_by_task = {k: torch.stack(v, dim=0) for k, v in corrects_knowful_by_task.items()}
        acc_knowful_by_task = {k: v.float().mean().item() for k, v in corrects_knowful_by_task.items()}
        acc_knowful_unweighted = np.mean(list(acc_knowful_by_task.values()))
        ...
        if not self.args.nolog and accelerator.is_main_process:
            stats = {
                ...
                'eval/acc_knowful_unweighted': acc_knowful_unweighted,
            }
        ...
        #     self.eval_accs[step] = acc_knowful_unweighted
        #     if prev_best_step is None or acc_knowful_unweighted > self.eval_accs[prev_best_step]:
        #         if prev_best_step is not None:
        #             try:
        #                 os.remove(f'{self.args.model_dir}/ckp_{prev_best_step}.pth')
        #             except:
    ...
    def save(self, step):
        ...
        if self.args.save_interval != 0:
            ...
            if step % self.args.qk_and_qka_steps not in [0]: # only save after qka steps
                return
...
def get_args():
    ...
    # common
    parser.add_argument('--model_type', type=str, default='t5-large', choices=['t5-large', 't5-3b', 't5-11b'])
    ...
    parser.add_argument('--engine', type=str, default='davinci', choices=['curie', 'davinci'])
